...
// for shareded queue
template <typename Queue>
void runThroughputSharded(std::int64_t consumerCount, std::int64_t itemsCount) {
    Queue q;
    std::atomic<std::int64_t> consumed{0};
    std::vector<std::uint64_t> checksums(
        static_cast<std::size_t>(consumerCount), 0);
    ...
    for (std::int64_t i = 0; i < consumerCount; ++i) {
        consumers.emplace_back([&, i]() {
            std::uint64_t local = 0;
            hdr_histogram* h = hists[i];
            auto myRing = q.getShard(static_cast<std::uint64_t>(i));
            while (consumed.load(std::memory_order_relaxed) < itemsCount) {
                auto start = __rdtsc();
                auto v = myRing->pop();   // own shard
                auto dt = __rdtsc() - start;
                if (v) {
                    local += *v;
                    consumed.fetch_add(1, std::memory_order_relaxed);
                    hdr_record_value(h, dt);
    ...
    const auto start = std::chrono::steady_clock::now();
    for (std::int64_t i = 0; i < itemsCount; ++i)
        while (!q.push(static_cast<std::uint64_t>(i)))   // queue distributes internally
            ;
    for (auto& c : consumers)
        c.join();
    const auto end = std::chrono::steady_clock::now();
